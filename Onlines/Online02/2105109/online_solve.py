...
stock_prices.plot()
Unweighted_Window_signal=DiscreteSignal(len(price_list))
for i in range(n):
    Unweighted_Window_signal.set_value_at_time(i,(i+1)/n)
Unweighted_Window_signal.plot()
A=LTI_Discrete(Unweighted_Window_signal)
[outputs,sum_signal]=A.output(stock_prices)
sum_signal.plot()
weighted_Window_signal=DiscreteSignal(len(price_list))
s=(n*(n+1))/2
for i in range(n):
