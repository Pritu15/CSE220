...
def IFFT(fft_signal) : 
    n = len(fft_signal)
    conj_fft_signal = np.conj(fft_signal)
    time_signal = FFT(conj_fft_signal)
...
def generate_signal(frequency=5):
...
signal_a = np.random.rand(2**4)
x_values = np.arange(0,2**4,1)

plt.plot(x_values, signal_a)
plt.show()

# FFT(signal_a)

plt.plot(x_values, IFFT(FFT(signal_a)) )
plt.show()

# plot_original_signal(signal_a,"signal a ","red")
# plot_original_signal(IFFT(FFT(signal_a)),"reconstructed signal a ", "green")
